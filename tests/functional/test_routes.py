...
def test_delete_account(testing_client):
    ...
    assert create_response.status_code == 200, f"Failed to create account. Status: {create_response.status_code}, Response: {create_response.data}"

    account_data = create_response.get_json()
    print(f"Create response data: {account_data}")  # Debug print
    ...
    assert delete_response.status_code == 200, f"Failed to delete account. Status: {delete_response.status_code}, Response: {delete_response.data}"

    # Verify that the account was deleted
    get_response = testing_client.get(f'/accounts/{account_id}')
    assert get_response.status_code == 404, f"Expected 404, got {get_response.status_code}. Response: {get_response.data}"
